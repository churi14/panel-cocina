...
export default function KitchenProductionModal({ onClose, activeProductions, setActiveProductions, recipesDB, setProductionHistory, operadorNombre }: {
  ...
  // SalsaModal is rendered inline (not as component) to avoid re-mount focus loss


  // EmpanadoModal - rendered inline in JSX below
        ...
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.6)',padding:'20px'}}>
          <div style={{background:'white',borderRadius:'20px',width:'100%',maxWidth:'360px',padding:'24px',boxShadow:'0 25px 50px rgba(0,0,0,0.3)'}}>
            ...
            <div style={{display:'flex',gap:'12px'}}>
              <button onClick={() => setShowSalsaModal(false)}
                style={{flex:1,padding:'12px',border:'2px solid #e2e8f0',borderRadius:'12px',fontWeight:'700',color:'#64748b',background:'white',cursor:'pointer',fontSize:'15px'}}>
                Cancelar
              </button>
              <button onClick={() => handleFinish()}
      ...
      {showEmpanadoModal && finishingProd && (
        <div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.6)',padding:'20px'}}>
          <div style={{background:'white',borderRadius:'20px',width:'100%',maxWidth:'400px',padding:'24px',boxShadow:'0 25px 50px rgba(0,0,0,0.3)',maxHeight:'85vh',overflowY:'auto'}}>
              ...
              {empanadoStocks.length > 0 ? (
                <div style={{display:'flex',flexDirection:'column',gap:'6px',maxHeight:'140px',overflowY:'auto'}}>
                  {empanadoStocks.map(s => (
                    <button key={s.producto}
                      onClick={() => { setEmpanadoCorteStock(s.producto); setEmpanadoTipo(s.producto.toLowerCase().includes('pollo') ? 'pollo' : 'carne'); }}
                ...
                <p style={{fontSize:'12px',color:'#f43f5e',background:'#fff1f2',padding:'10px',borderRadius:'8px',border:'1px solid #fecdd3'}}>⚠️ Sin menjunje disponible.</p>
              )}
            </div>
            <div style={{marginBottom:'12px'}}>
            ...
            <div style={{display:'flex',gap:'12px'}}>
              <button onClick={() => setShowEmpanadoModal(false)}
                style={{flex:1,padding:'12px',border:'2px solid #e2e8f0',borderRadius:'12px',fontWeight:'700',color:'#64748b',background:'white',cursor:'pointer',fontSize:'15px'}}>
                Cancelar
              </button>
              <button onClick={() => handleFinish()}
                disabled={!empanadoMenjunjeKg || !empanadoSalieronKg || !empanadoCorteStock}
