...
async function getVapidKeys() {
  ...
  const privateKeyBytes = base64UrlDecode(privKey);
  const publicKeyBytes  = base64UrlDecode(pubKey);
  ...
  return { pubKey, publicKeyBytes, cryptoKey };
}

async function buildVapidAuthHeader(endpoint: string, pubKey: string, cryptoKey: CryptoKey) {
  ...
  const sigKey = await crypto.subtle.importKey(
    'raw', base64UrlDecode(process.env.VAPID_PRIVATE_KEY!),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const payload = JSON.stringify({ title, body, tag: tag ?? 'kitchenos', url: notifUrl ?? '/' });
    const pubKey  = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY!;
    ...
    for (const sub of subs) {
      try {
        const authHeader = await buildVapidAuthHeader(sub.endpoint, pubKey, null as any);
