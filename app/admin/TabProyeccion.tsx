...
export default function TabProyeccion() {
  ...
  const [tab, setTab] = useState<'semana' | 'horas' | 'tendencia' | 'produccion'>('produccion');
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex bg-slate-800 border border-slate-700 p-1 rounded-xl gap-1 w-fit">
        {([['produccion', '📦 Producción'], ['semana', '📅 Semana'], ['horas', '⏰ Horario'], ['tendencia', '📈 Tendencia']] as const).map(([id, label]) => (
