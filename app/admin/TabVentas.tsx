    ...
    try {
      ...
      for (const d of stockDescuentos) {
        ...
        const { error: updateErr } = await supabase.from(tabla)
          .update({ cantidad: parseFloat(newQty.toFixed(3)), fecha_actualizacion: fecha })
          .eq('id', data.id);
